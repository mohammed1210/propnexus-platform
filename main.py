...
root = os.path.dirname(os.path.abspath(__file__))  # e.g. /app in Railway
...
# Ensure repo root on sys.path (nice to have, but not relied on)
if root not in sys.path:
    sys.path.insert(0, root)

# ---- CRUCIAL BIT ----
# Synthesize a package named "backend" so absolute imports like
# "from backend.routes import ..." work when we load backend/main.py by path.
backend_dir = os.path.join(root, "backend")
if "backend" not in sys.modules:
    pkg = ModuleType("backend")
    # pkg.__file__ is optional, but __path__ is required so submodules can be found
