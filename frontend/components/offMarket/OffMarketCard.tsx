...
export default function OffMarketCard({ deal }: Props) {
  ...
  const discount =
    d.discount_percent != null ? `${d.discount_percent.toFixed(0)}% off` : null;

  // Decide when to let Next optimize. Supabase images are usually fine to optimize.
  const shouldOptimize =
    typeof img === "string" && img.length > 0 && img.includes("supabase");
