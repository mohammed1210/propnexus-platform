...
const BANDS_STANDARD: Band[] = [
  { upTo: 250_000, rate: 0.00, label: 'Up to £250k' },
  { upTo: 925_000, rate: 0.05, label: '£250k–£925k' },
  { upTo: 1_500_000, rate: 0.10, label: '£925k–£1.5m' },
  { upTo: null, rate: 0.12, label: '£1.5m+' },
];

// First-time buyer relief (England & NI): 0% up to 425k, 5% on 425k–625k, over 625k -> standard
const BANDS_FTB: Band[] = [
  { upTo: 425_000, rate: 0.00, label: 'Up to £425k (FTB relief)' },
  { upTo: 625_000, rate: 0.05, label: '£425k–£625k (FTB relief)' },
  // over £625k -> standard rates (we’ll swap bands dynamically)
];
...
export default function StampDutyCalculator({ price, className }: StampDutyCalculatorProps) {
  ...
  // Keep in sync if parent price changes
  useEffect(() => setInputPrice(price), [price]);

  const baseline = useMemo(() => {
    // First-time buyer logic: if > 625k, no relief -> standard bands
    if (buyerType === 'first_time') {
      if (inputPrice <= 625_000) {
        return calcFromBands(inputPrice, BANDS_FTB);
      }
      // no relief above £625k
      return calcFromBands(inputPrice, BANDS_STANDARD);
    }
    return calcFromBands(inputPrice, BANDS_STANDARD);
  }, [buyerType, inputPrice]);

  // Additional property surcharge: +3% on **all** bands (effectively +3% of whole price)
  const surcharge = buyerType === 'additional' ? inputPrice * 0.03 : 0;
  const totalDuty = Math.max(0, Math.round(baseline.total + surcharge));
  ...
  return (
    <div className={`bg-white dark:bg-neutral-900 shadow-md rounded-md p-5 mt-6 ${className ?? ''}`}>
          ...
          <tbody>
            {baseline.rows.map((r, i) => (
              <tr key={i} className="border-t border-neutral-200 dark:border-neutral-800">
                ...
                <td className="py-1 pr-3">{fmt(r.taxable)}</td>
                <td className="py-1 pr-3">{r.ratePct.toFixed(0)}%</td>
                <td className="py-1">{fmt(Math.round(r.duty))}</td>
              </tr>
            ))}
            {buyerType === 'additional' && (
              <tr className="border-t border-neutral-200 dark:border-neutral-800">
                <td className="py-1 pr-3">Surcharge (+3%)</td>
                <td className="py-1 pr-3">{fmt(inputPrice)}</td>
                <td className="py-1 pr-3">3%</td>
                <td className="py-1">{fmt(Math.round(surcharge))}</td>
              </tr>
            )}
            <tr className="border-t border-neutral-300 dark:border-neutral-700 font-semibold">
              ...
              <td className="py-1 pr-3" />
              <td className="py-1">{fmt(totalDuty)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <p className="text-xs text-gray-500 mt-3">
        England & Northern Ireland residential rates. This is an estimate — surcharges/reliefs can vary by circumstances.
      </p>
    </div>
  );
}