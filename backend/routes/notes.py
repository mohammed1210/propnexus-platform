...
# support either var name, but prefer SUPABASE_SERVICE_ROLE
...
class NotesPayload(BaseModel):
    property_id: str = Field(..., description="Property UUID/text identifier")
    user_id: Optional[str] = Field(
        default=None, description="User identifier (optional for now)"
    )
...
# --- Helpers -----------------------------------------------------------------------
def _now_iso() -> str:
    # Always store UTC in ISO-8601 with 'Z'
    return datetime.now(timezone.utc).isoformat()


def _norm_user(user_id: Optional[str]) -> str:
    # Your current convention: null/None becomes empty string
    return user_id or ""
...
def _raise_if_error(resp) -> None:
    # supabase-py v2 puts errors on resp.error; resp.data is list/dict
...
@router.get(
    ...
    summary="Fetch notes for a property (optionally scoped to user_id)",
)
def get_notes(property_id: str, user_id: Optional[str] = Query(default=None)):
    ...
    if resp.data:
        ...
        row = resp.data[0]
        return NotesRecord(**row)

    # return an empty shell if nothing exists yet
...
@router.get(
    ...
    summary="List notes for a user_id (all properties)",
)
...
def patch_notes(
    ...
    # Update row; if not found, create it with provided fields
    # 1) Try update
    resp = (
        sb.table("notes")
        .update(updates)
    ...
    # 2) If no rows updated, insert a new one (upsert behavior for patch)
    if not resp.data:
        base = {
            "property_id": property_id,
            "user_id": uid,
        ...
        resp2 = (
            sb.table("notes").upsert(base, on_conflict="user_id,property_id").execute()
        )
