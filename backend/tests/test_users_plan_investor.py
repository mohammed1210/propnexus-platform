...
def test_webhook_investor_plan_checkout():
    ...
    with patch.dict("os.environ", {"STRIPE_WEBHOOK_SECRET": "test_secret", "STRIPE_PRICE_INVESTOR": "price_investor_123"}):
        # Mock Stripe webhook verification
        with patch("backend.routes.stripe_webhook.stripe.Webhook.construct_event") as mock_construct:
            ...
            with patch("backend.routes.stripe_webhook.stripe.Subscription.retrieve") as mock_retrieve:
                mock_retrieve.return_value = {
                    "status": "active",
                    "current_period_end": 1735689600,  # 2025-01-01
                    "items": {
                        "data": [
                            {"price": {"id": "price_investor_123"}}
                        ]
                    },
                }
...
def test_webhook_pro_plan_checkout():
    ...
    with patch.dict("os.environ", {"STRIPE_WEBHOOK_SECRET": "test_secret", "STRIPE_PRICE_PRO": "price_pro_123"}):
        with patch("backend.routes.stripe_webhook.stripe.Webhook.construct_event") as mock_construct:
            mock_construct.return_value = {
                "type": "checkout.session.completed",
                "data": {
        ...
        with patch("backend.routes.stripe_webhook.stripe.Webhook.construct_event") as mock_construct:
            ...
            with patch("backend.routes.stripe_webhook.stripe.Subscription.retrieve") as mock_retrieve:
                mock_retrieve.return_value = {
                    ...
                    "items": {
                        "data": [
                            {"price": {"id": "price_unknown_xyz"}}  # Unknown price_id
                        ]
                    },
                }
