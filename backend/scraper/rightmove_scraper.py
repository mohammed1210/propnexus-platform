        ...
        for page in range(RM_MAX_PAGES):
            ...
            for card in cards:
                if len(results) >= limit:
                    break
                try:
                    title_el = card.select_one(".propertyCard-title") or card.select_one("[data-testid='title']") or card.select_one("h2")
                    title = title_el.get_text(strip=True) if title_el else "Untitled"

                    price_el = card.select_one(".propertyCard-priceValue") or card.select_one("[data-testid='price']")
                    price = _parse_price(price_el.get_text(strip=True) if price_el else "")

                    loc_el = card.select_one(".propertyCard-address") or card.select_one("[data-testid='address']") or card.select_one(".address")
                    location_text = loc_el.get_text(" ", strip=True) if loc_el else location

                    beds_el = card.select_one("[data-testid='bedrooms']") or card.select_one(".beds") or card.select_one(".propertyCard-description")
                    bedrooms = _extract_int(beds_el.get_text() if beds_el else "") or 0

                    baths_el = card.select_one("[data-testid='bathrooms']") or card.select_one(".baths")
                    ...
                    external_id = _extract_property_id(card) or f"rm-{hash(title+location_text) & 0xffffffff}"
