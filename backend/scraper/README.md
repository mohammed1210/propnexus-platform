...
- Railway uses `bash scripts/railway-start.sh` as the repo start command. It dispatches API services to `uvicorn backend.main:app` and the `function-bun`/`vivacious-embrace` ingest-worker service to `python -m backend.tasks.ingestion_runner`.
